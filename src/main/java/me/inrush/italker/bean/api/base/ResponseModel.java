package me.inrush.italker.bean.api.base;
...
public class ResponseModel<M> implements Serializable {
    ...
    public static <M> ResponseModel<M> buildNotFoundGroupError(String str) {
        return new ResponseModel<>(ERROR_NOT_FOUND_GROUP, str != null ? str : "Not Found Group.");
    }

    public static <M> ResponseModel<M> buildNotFoundGroupMemberError(String str) {
        return new ResponseModel<>(ERROR_NOT_FOUND_GROUP_MEMBER, str != null ? str : "Not Found GroupMember.");
    }

    public static <M> ResponseModel<M> buildAccountError() {
        return new ResponseModel<>(ERROR_ACCOUNT_TOKEN, "Account Error; you need login.");
    }

    public static <M> ResponseModel<M> buildLoginError() {
        return new ResponseModel<>(ERROR_ACCOUNT_LOGIN, "Account or password error.");
    }
    ...
    public static <M> ResponseModel<M> buildNoPermissionError() {
        return new ResponseModel<>(ERROR_ACCOUNT_NO_PERMISSION, "You do not have permission to operate.");
    }

    public static <M> ResponseModel<M> buildCreateError(int type) {
        return new ResponseModel<>(type, "Create failed.");
    }
